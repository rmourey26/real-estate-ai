...
import { repliersClient, type RepliersMarketInsight, type RepliersPropertyAnalysis } from "./clients/repliers"
\
import type
\
{
  PropertyDetails, PropertyValuation, MarketTrend, NeighborhoodInfo, PropertySearchParams
  \
}
from
;("./types")
...
\
{
  RepliersMarketInsight, RepliersPropertyAnalysis, RepliersOpportunityZone
  \
}
...
\
  ...
  \
  {
    ...
    const result = \
    propertyId, estimatedValue
    : Math.round(baseValue * 1.02), // Slightly higher than listing price
      valuationRange: \
    low: Math.round(baseValue * 0.95), high
    : Math.round(baseValue * 1.08),
      \
    ,
...
\}): Promise<NeighborhoodInfo> \
{
  ...
  \
  {
    ...
    overview:
    \
    population: data.overview?.population || 45000, medianIncome
    : data.overview?.medianIncome || 85000,
    ...
    : school.grades,
            ...
            amenities: \
    restaurants: data.amenities?.restaurants || 42, groceryStores
    : data.amenities?.groceryStores || 8,
    ...
    ,
            transportation: \
    walkScore: data.transportation?.walkScore || 72, transitScore
    : data.transportation?.transitScore || 65,
              bikeScore: data.transportation?.bikeScore || 68,
              averageCommute: data.transportation?.averageCommute || 28,
            \
    ,
            crimeRate: \
    overall: data.crimeRate?.overall || "Low", violent
    : data.crimeRate?.violent || "Very Low",
              property: data.crimeRate?.property || "Low",
              comparedToNational: data.crimeRate?.comparedToNational || "-15%",
            \
    ,
            marketTrends: \
    homeValueTrend: data.marketTrends?.homeValueTrend || "+4.2% YoY", forecastNextYear
    : data.marketTrends?.forecastNextYear || "+3.8%",
  ...
  ,
      schools: [
        \
  name: "Washington Elementary", type
  : "Public",
          grades: "K-5",
          rating: 8.5,
          distance: 0.8,
  ...
  ,
        \
  name: "Lincoln Middle School", type
  : "Public",
          grades: "6-8",
          rating: 7.9,
          distance: 1.2,
  ...
  ,
        \
  name: "Roosevelt High School", type
  : "Public",
          grades: "9-12",
          rating: 8.2,
          distance: 1.5,
  ...
  ,
      marketTrends: \
  homeValueTrend: "+4.2% YoY", forecastNextYear
  : "+3.8%",
...
// Helper function to generate mock market insights
function getMockMarketInsights(params: \{ region: string;
regionType?: string
\}): RepliersMarketInsight \
{
  return \
  region: params.region, regionType
  : params.regionType || "city",
    period: `$\
  new Date().getMonth() + 1
  \
  ;/ $(()),.DFY\\`aaeeeegllnrttuw{}
  metrics:
  \
  medianPrice: 450000, priceChangePct
  : 3.5,
      avgDaysOnMarket: 22,
      inventoryCount: 350,
      salesVolume: 120,
  ...
  shortTerm:
  \
  priceChangePct: 1.2, confidence
  : 85,
      \
  ,
      mediumTerm: \
  priceChangePct: 3.8, confidence
  : 75,
      \
  ,
      longTerm: \
  priceChangePct: 12.5, confidence
  : 65,
      \
  ,
    \
...
\}): Promise<RepliersMarketInsight> \
{
  ...
  // Immediately return mock data if API key is missing
  if (!config.features.useRealApis || !config.repliers.apiKey)
  \
  {
    console.warn("Repliers API key not configured. Falling back to mock data for getMarketInsights.")
    const mockInsights = getMockMarketInsights(params)
  ...
  {
    console.error(`Error in getMarketInsights: $\{error\}. Falling back to mock data.`)
    // If API call fails (e.g., 401), return mock data
    const mockInsights = getMockMarketInsights(params)
    setCache(cacheKey, mockInsights)
    return mockInsights
    \
  ...
  try \{
    if (config.features.useRealApis && config.repliers.apiKey) \
      try \{
        ...
        return analysis
      \} catch (apiError) \
        console.error(`Repliers API error in getPropertyAnalysis: $\{apiError\}. Falling back to mock data.`)
        // Fallback to mock data on API error
      \
    \

    // Get property details from database for mock data generation
    const supabase = createClient()
    const \{ data: property, error \} = await supabase
      .from("real_estate_listings")
      .select("*")
    ...
    if (error || !property) \
      throw new Error("Property not found for mock analysis generation")
    \
...
\}): Promise<RepliersOpportunityZone[]> \{
  ...
  if (!config.features.useRealApis || !config.repliers.apiKey) \{
    console.warn("Repliers API key not configured. Falling back to mock data for getOpportunityZones.")
  ...
  try \{
    ...
    return zones
  \} catch (error) \{
    console.error(`Error in getOpportunityZones: $\error\. Falling back to mock data.`)
    setCache(cacheKey, mockZones)
    return mockZones
  \
\
